...
/**
  ...
  */
class Cross(data: Option[DataFrame], loadPath: Option[String], savePath: Option[String])
  ...
  override def execute(implicit sc: SparkContext, sqlContext: SQLContext): Unit = {
    ...
    val paramGrid = new ParamGridBuilder()
      .addGrid(knn.topTreeSize, 2 to 8 by 2)
      .addGrid(knn.topTreeLeafSize, 2 to 10 by 2)
      .addGrid(knn.topTreeLeafSize, 2 to 10 by 2)
      .build()
    ...
    val predictionTrain = cvModel.transform(train)
    val predictionTest = cvModel.transform(test)
