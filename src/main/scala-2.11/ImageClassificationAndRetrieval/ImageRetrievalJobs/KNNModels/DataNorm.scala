...
/**
  ...
  */
class DataNorm(data: Option[DataFrame], loadPath: Option[String], savePath: Option[String])
  ...
  override def execute(implicit sc: SparkContext, sqlContext: SQLContext): Unit = {
    ...
    val normalizedData = normalizer1
      .transform(assembledData)

    val stdScaler1 = new StandardScaler()
      .setInputCol("features")
      .setOutputCol("scaledFeatures")
      .setWithMean(true)
      .setWithStd(true)

    val scaledData = stdScaler1
      .fit(assembledData)
      .transform(assembledData)

    //split traning and testing
    val Array(train, test) = assembledData //TODO use with StdData & NormData
